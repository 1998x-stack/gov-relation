...
- **SQLite** is the production system of record for canonical entities, assertions, review decisions and release manifests, stored in the single unified database `data/platform/gov_relation.db`.
- **Object storage** retains fetched documents, normalized text, checksums and capture metadata. Content stays immutable; a corrected fetch creates a new asset.
- **Search** starts with SQLite indexes (including FTS5). Add a dedicated search engine only after measured query/SLA pressure.
- **Graph** queries use relational edges initially. Add a graph projection only for proven multi-hop workloads.

Keep high-volume append-only tables (`source_assets`, `assertions`, `audit_events`) with time-ordered indexes and archive old partitions by capture/release time after scale measurements. Tenant-facing access is enforced through release entitlements and application-level authorization, not by database row policies.
...
- SQLite is the embedded storage engine (<https://www.sqlite.org/>); FTS5 powers full-text search over released text.
