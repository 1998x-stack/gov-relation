#!/usr/bin/env python3
"""Build SQLite database, GEXF graph, and person JSONs for 久治县 (Jigzhi County), 果洛藏族自治州, 青海省.
...
Research status: PARTIAL — web access severely degraded during investigation:
  ...
  - Government websites (久治县 jiuzhixian.gov.cn, 果洛州 guoluo.gov.cn) unreachable via HTTP(S) and Jina Reader
...
Confidence notes:
  ...
  - The official county website is www.jiuzhixian.gov.cn (confirmed via Wikipedia).
  - The parent prefecture website is www.guoluo.gov.cn (confirmed).
  - Current officeholder names are marked as "待查_县委书记" and "待查_县长"
    (to be checked) — verifying from the 久治县 government website's 领导之窗 page
...
Expected government website: www.jiuzhixian.gov.cn
...
_STAGING_CANDIDATE = REPO_ROOT / "data" / "tmp" / "qinghai_久治县"
if _CURRENT_DIR.name == "qinghai_久治县":
...
# Expected domain: www.jiuzhixian.gov.cn
...
persons = [
    ...
    # --- 县委书记 (Party Secretary) ---
    {
        ...
        "work_start": "",
        "current_post": "久治县委书记",
        "current_org": "中共久治县委员会",
        "source": "Web access degraded — name unverified. Check 久治县领导之窗 at www.jiuzhixian.gov.cn",
    },
    # --- 县长 (County Chief) ---
    {
        "id": 2,
    ...
    # --- 县委常委、纪委书记、监委主任 ---
    {
        ...
        "party_join": "中共党员",
        "work_start": "",
        "current_post": "久治县委常委、纪委书记、监委主任",
        "current_org": "中共久治县纪律检查委员会",
        "source": "Web access degraded — name unverified.",
    },
    # --- 县委常委、组织部部长 ---
    {
        ...
        "party_join": "中共党员",
        "work_start": "",
        "current_post": "久治县委常委、组织部部长",
        "current_org": "中共久治县委组织部",
        "source": "Web access degraded — name unverified.",
    },
    # --- 县委常委、宣传部部长 ---
    {
        ...
        "party_join": "中共党员",
        "work_start": "",
        "current_post": "久治县委常委、宣传部部长",
        "current_org": "中共久治县委宣传部",
        "source": "Web access degraded — name unverified.",
    },
    # --- 县委常委、政法委书记 ---
    {
        ...
        "party_join": "中共党员",
        "work_start": "",
        "current_post": "久治县委常委、政法委书记",
        "current_org": "中共久治县委政法委员会",
        "source": "Web access degraded — name unverified.",
    },
    # --- 县委常委、统战部部长 ---
    {
        ...
        "party_join": "中共党员",
        "work_start": "",
        "current_post": "久治县委常委、统战部部长",
        "current_org": "中共久治县委统一战线工作部",
        "source": "Web access degraded — name unverified.",
    },
    # --- 县委常委、县委办公室主任 ---
    {
        ...
        "party_join": "中共党员",
        "work_start": "",
        "current_post": "久治县委常委、县委办公室主任",
        "current_org": "中共久治县委办公室",
        "source": "Web access degraded — name unverified.",
    },
    # ════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════
    {
        ...
        "party_join": "中共党员",
        "work_start": "",
        "current_post": "久治县人大常委会主任",
        "current_org": "久治县人民代表大会常务委员会",
        "source": "Web access degraded — name unverified.",
    },
    # ════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════
    {
        ...
        "party_join": "中共党员",
        "work_start": "",
        "current_post": "久治县政协主席",
        "current_org": "中国人民政治协商会议久治县委员会",
        "source": "Web access degraded — name unverified.",
    },
    # ════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════
    {
        ...
        "party_join": "中共党员",
        "work_start": "",
        "current_post": "久治县人民检察院检察长",
        "current_org": "久治县人民检察院",
        "source": "Web access degraded — name unverified.",
    },
]
...
positions = [
    # 县委书记
    {"person_id": 1, "org_id": 1, "title": "县委书记", "start_date": "", "end_date": "present", "rank": "正处级", "note": "久治县一把手"},
    # 县长（兼县委副书记）
...
relationships = [
    # 县委书记 — 县长（党政正职关系）
    {"person_a": 1, "person_b": 2, "type": "overlap", "context": "县委书记—县长党政搭档", "overlap_org": "久治县四套班子", "overlap_period": "截至2026年"},
    # 县委书记 — 县委副书记
    {"person_a": 1, "person_b": 3, "type": "superior_subordinate", "context": "县委书记—副书记", "overlap_org": "中共久治县委员会", "overlap_period": "截至2026年"},
    # 县委书记 — 纪委书记
    {"person_a": 1, "person_b": 5, "type": "superior_subordinate", "context": "县委书记—纪委书记（同级监督关系）", "overlap_org": "中共久治县委员会", "overlap_period": "截至2026年"},
    # 县委书记 — 组织部长
    {"person_a": 1, "person_b": 6, "type": "superior_subordinate", "context": "县委书记—组织部长（干部管理）", "overlap_org": "中共久治县委员会", "overlap_period": "截至2026年"},
    # 县委常委间同僚关系
    {"person_a": 4, "person_b": 5, "type": "overlap", "context": "县委常委同僚", "overlap_org": "中共久治县委员会", "overlap_period": "截至2026年"},
    {"person_a": 4, "person_b": 6, "type": "overlap", "context": "县委常委同僚", "overlap_org": "中共久治县委员会", "overlap_period": "截至2026年"},
    {"person_a": 4, "person_b": 7, "type": "overlap", "context": "县委常委同僚", "overlap_org": "中共久治县委员会", "overlap_period": "截至2026年"},
    {"person_a": 4, "person_b": 8, "type": "overlap", "context": "县委常委同僚", "overlap_org": "中共久治县委员会", "overlap_period": "截至2026年"},
    {"person_a": 4, "person_b": 9, "type": "overlap", "context": "县委常委同僚", "overlap_org": "中共久治县委员会", "overlap_period": "截至2026年"},
    {"person_a": 4, "person_b": 10, "type": "overlap", "context": "县委常委同僚", "overlap_org": "中共久治县委员会", "overlap_period": "截至2026年"},
    # 县长—常务副县长（政府班子）
    {"person_a": 2, "person_b": 4, "type": "superior_subordinate", "context": "县长—常务副县长", "overlap_org": "久治县人民政府", "overlap_period": "截至2026年"},
    # 县长—副县长
    {"person_a": 2, "person_b": 11, "type": "superior_subordinate", "context": "县长—副县长", "overlap_org": "久治县人民政府", "overlap_period": "截至2026年"},
    {"person_a": 2, "person_b": 12, "type": "superior_subordinate", "context": "县长—副县长", "overlap_org": "久治县人民政府", "overlap_period": "截至2026年"},
]
...
def write_person_json(person: dict, job: str) -> None:
    ...
    data = {
        ...
        "identity": {
            "person_id": f"qinghai_guoluo_jiuzhi_{safe_name}_unknown",
            "name": person["name"],
            "aliases": [],
        ...
        "open_questions": [
            ...
            },
            {
                ...
                "suggested_queries": [f"久治县 {person['name']} 简历", f"{person['name']} 任职履历"],
                "last_attempted": AS_OF,
            },
        ],
    }
