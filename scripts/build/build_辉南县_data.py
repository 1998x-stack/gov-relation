#!/usr/bin/env python3
"""Build SQLite database, GEXF graph, and person JSONs for 辉南县, 通化市, 吉林省.
...
Research sources:
  - www.huinan.gov.cn — 辉南县人民政府官方网站 (unreachable — timed out)
  - www.tonghua.gov.cn — 通化市人民政府官方网站 (unreachable — timed out)
...
Confidence notes:
  ...
  - Government sites (huinan.gov.cn, tonghua.gov.cn) all timed out.
...
_STAGING_CANDIDATE = REPO_ROOT / "data" / "tmp" / "jilin_辉南县"
if _CURRENT_DIR.name == "jilin_辉南县":
...
persons = [
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "work_start": "",
        "current_post": "县委书记",
        "current_org": "中共辉南县委员会",
        "source": "待查 — 辉南县政府官网(huinan.gov.cn)无法访问",
        "confidence": "unverified",
        "notes": "县委书记姓名确认失败。县政府网站huinan.gov.cn及所有子页面均超时无法访问。通化市政府网站同样不可达。未找到任何可靠的公开来源确认当前在任县委书记姓名。需后续通过通化市委组织部公示或新闻报道进一步核实。"
    },
    {
        "id": 2,
        "name": "待查_县长",
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "confidence": "unverified",
        "notes": "常务副县长姓名待核实。县级政府一般配备一名常务副县长。具体信息需从辉南县政府网站领导分工页面获取。"
    },
    {
        "id": 4,
        "name": "待查_纪委书记",
...
# ── Organizations ─────────────────────────────────────────────────────────────
organizations = [
    {"id": 1, "name": "中共辉南县委员会", "type": "党委", "level": "县处级", "parent": "中共通化市委", "location": "辉南县"},
    {"id": 2, "name": "辉南县人民政府", "type": "政府", "level": "县处级", "parent": "通化市人民政府", "location": "辉南县"},
    {"id": 3, "name": "中国人民政治协商会议辉南县委员会", "type": "政协", "level": "县处级", "parent": "政协通化市委", "location": "辉南县"},
    {"id": 4, "name": "辉南县人民代表大会常务委员会", "type": "人大", "level": "县处级", "parent": "通化市人大常委会", "location": "辉南县"},
    {"id": 5, "name": "中共辉南县纪律检查委员会（监察委员会）", "type": "纪委", "level": "县处级", "parent": "通化市纪委", "location": "辉南县"},
    {"id": 6, "name": "中共辉南县委组织部", "type": "党委", "level": "县处级", "parent": "中共辉南县委员会", "location": "辉南县"},
    {"id": 7, "name": "中共辉南县委宣传部", "type": "党委", "level": "县处级", "parent": "中共辉南县委员会", "location": "辉南县"},
    {"id": 8, "name": "中共辉南县委政法委员会", "type": "党委", "level": "县处级", "parent": "中共辉南县委员会", "location": "辉南县"},
    {"id": 9, "name": "辉南县公安局", "type": "政府", "level": "乡科级", "parent": "辉南县人民政府", "location": "辉南县"},
]
...
def build_person_json(person_id: int) -> dict:
    ...
    person = {
        "identity": {
            "person_id": f"jilin_tonghua_huinan_{name}",
            "name": name,
            "aliases": [],
            "gender": p["gender"],
        ...
        "open_questions": [
            {
                "priority": "critical",
                "question": f"辉南县{role_label}姓名是什么？",
                "why_it_matters": "核心目标人物之一，完整调查必须确认姓名和身份",
                "suggested_queries": [
                    f"辉南县 {p['current_post']}",
                    f"辉南县 领导分工 {p['current_post']}",
                    "辉南县政府 领导之窗",
                    "通化市委组织部 任前公示 辉南",
                ],
...
def write_person_json(person_id: int):
    ...
    filename = f"{TODAY}-吉林省-通化市-{safe_role}-{name}.json"
...
def main():
    ...
    print(f"      Government sites (huinan.gov.cn) timed out; Baidu 403; Exa rate-limited.")
    print(f"Done.")
